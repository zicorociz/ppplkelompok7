...
import 'package:stay_place/helpers/widgets/my_bottom_navigation_bar_item.dart';
...
class _MyBottomNavigationBarState extends State<MyBottomNavigationBar> with SingleTickerProviderStateMixin {
                      ...
                      ? Text(
                          ...
                          style: activeTitleStyle ??
                              ...
                              MyTextStyle.labelSmall(
                                  color: activeTitleColor ?? item.activeTitleColor ?? theme.primaryColor, fontSize: activeTitleSize ?? item.activeTitleSize),
                        )
                      : Container(),
                ],
              )
                  ...
                  showLabel
                      ? Text(
                          ...
                          style: titleStyle ??
                              item.titleStyle ??
                              MyTextStyle.labelSmall(
                                  color: titleColor ?? item.titleColor ?? theme.colorScheme.onSurface, fontSize: widget.titleSize ?? item.titleSize),
                        )
                      : Container(),
                ],
              ),
  ...
  List<Widget> getListOfItems() {
    ...
    double singleWidth = (MediaQuery.of(context).size.width - 50) / (itemList!.length + (widget.showLabel ? 0 : (showActiveLabel ? 0.5 : 0)));

    for (int i = 0; i < itemList!.length; i++) {
      double containerWidth = widget.showLabel ? (singleWidth) : (showActiveLabel ? (i == _currentIndex ? singleWidth * 1.5 : singleWidth) : singleWidth);
      list.add(SizedBox(
